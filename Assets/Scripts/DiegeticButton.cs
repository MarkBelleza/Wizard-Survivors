...
public class DiegeticButton : Interactable
{
    ...
    void Update()
    {
        if (touchingCtrl != null) // If we are touching button cap
        {
            ...
            Vector3 controllerLocalPos = parent.InverseTransformPoint(touchingCtrl.transform.position);
            ...
            Vector3 pressDir = -axisLocal;

            // --- TODO - Vector from the cap's rest position to the controller, in parent-local space
            Vector3 capToCont = controllerLocalPos - restLocalPos;

            // --- TODO - Signed distance along press direction (use dot product) 
            float signedDis = Vector3.Dot(capToCont, pressDir); //if (+): both vectors in the same direction, (-): both vectors in opposite dir, (0): both are perpendicular
            ...
            float adjustedDis = Mathf.Max(0f, signedDis - deadZone);
            pressDepth = Mathf.Clamp(adjustedDis, 0f, maxPressDepth);
